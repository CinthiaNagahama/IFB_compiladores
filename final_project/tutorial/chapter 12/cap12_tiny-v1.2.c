/*
  Capítulo 12 - Miscelâneas -> Tiny 1.2
*/
...
char *SymbolTable[SYMBOLTABLE_SIZE]; /* Tabela de sí­mbolos */
char SymbolType[SYMBOLTABLE_SIZE];   /* Tabela de tipos de sí­mbolos */
int SymbolCount;                     /* Número de entradas na tabela de sí­mbolos */
...
/* Reporta um erro se Token NíO for um identificador */
void CheckIdent()
{
  if (Token != 'x')
    Expected("Identifier");
...
/* Verifica se sí­mbolo está na tabela */
int InTable(char *name)
{
  return (Lookup(name, SymbolTable, SymbolCount) >= 0);
}

/* Retorna o endereço do identificador na tabela de sí­mbolos */
int Locate(char *name)
{
  return Lookup(name, SymbolTable, SymbolCount);
}

/* Reporta um erro se identificador NíO constar na tabela de sí­mbolos */
...
/* Reporta um erro se identificador JÁ constar na tabela de sí­mbolos */
...
/* Adiciona uma nova entrada à tabela de sí­mbolos */
...
/* Emite código para o epí­logo de um programa */
void AsmEpilog()
{
  EmitLn("MOV AX, 4C00h");
  EmitLn("INT 21h");
...
/* Reconhece um ponto-e-ví­rgula opcional */
void Semicolon()
{
  if (Token == ';')
    MatchString(";");
...
/* Remova o comentário para usar o estilo "Pascal" de ponto-e-ví­rgulas */
/* #define PASCAL_STYLE */
